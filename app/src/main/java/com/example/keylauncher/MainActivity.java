...
public class MainActivity extends AppCompatActivity
    ...
    /**
     * מפעיל/מכבה את מצב העכבר האמיתי של המערכת (Settings.Global
     * "mouse_support_list", בדיוק כמו מסך ההגדרות המקורי של הרום),
     * ואז מפעיל מחדש את הלאנצ'ר כדי שהשינוי ייכנס לתוקף.
     */
    private void toggleSystemMouseAndRestart() {

        boolean newState = SystemMouseBridge.toggleForPackage(
                getContentResolver(), getPackageName());

        // שומרים גם את המצב המקומי (משמש למשל את מסך ההגדרות)
        settings.mouse.setEnabled(newState);

        Toast.makeText(
                this,
                newState ? "מצב עכבר הופעל - מפעיל מחדש..." : "מצב עכבר כובה - מפעיל מחדש...",
                Toast.LENGTH_SHORT
        ).show();

        SystemMouseBridge.restartLauncher(this);
    }
